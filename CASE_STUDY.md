# Adding Cloud Dictation (Codex + Groq) to Handy
...
`build-custom.sh` captures the whole recipe — build with the right env, strip the hardened runtime, install to `/Applications` — so a rebuild is one command.
...
*Built on top of [cjpais/handy](https://github.com/cjpais/handy). All credit for Handy itself goes to its authors; this fork only adds the cloud backends and the x86_64 macOS build path.*
